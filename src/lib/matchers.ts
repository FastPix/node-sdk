...
import { FastpixDefaultError } from "../models/errors/fastpixdefaulterror.js";
...
export function match<T, E>(
  ...
  return async function matchFunc(
    ...
    [
      result: Result<T, E | FastpixDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new FastpixDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new FastpixDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
