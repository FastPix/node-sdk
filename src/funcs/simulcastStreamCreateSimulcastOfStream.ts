 ...
 */
export function simulcastStreamCreateSimulcastOfStream(
  client: FastpixCore,
  ...
  Result<
    ...
    | errors.SimulcastUnavailableError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateSimulcastOfStreamRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.simulcastRequest, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, models.SimulcastResponse$inboundSchema),
    M.jsonErr(400, errors.SimulcastUnavailableError$inboundSchema),
