 ...
 */
export function livePlaybackCreatePlaybackIdOfStream(
  client: FastpixCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreatePlaybackIdOfStreamRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.playbackIdRequest, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, models.PlaybackIdSuccessResponse$inboundSchema),
