...
export function liveStreamsDelete(
  ...
  Result<
    operations.DeleteLiveStreamResponse,MyError
  >
> {
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.DeleteLiveStreamRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    operations.DeleteLiveStreamResponse,MyError
  >(
    M.json(200, operations.DeleteLiveStreamResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.DeleteLiveStreamResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
