 ...
 */
export function manageLiveStreamUpdateLiveStream(
  client: FastpixCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateLiveStreamRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchLiveStreamRequest, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.PatchResponseDTO$inboundSchema),
